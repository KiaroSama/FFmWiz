"""The bounded reverse as it actually RUNS (D13).

`ext04c_run` executes the plan `ext04c` produced: the forward decode into
lossless chunks, the per-chunk reversal, the reverse-order concatenation, and
the final pass that re-points the original job at the reversed scratch. These
cases drive it through real FFmpeg on real fixtures and check the audio that
comes back, not the argv.

The budget is forced small by patching the SHARED peak-budget constants on
`ext04c`, so the real arithmetic still runs and a four-second fixture really
does produce ten chunks. The plan's own arithmetic is covered by
`test_bounded_audio_reverse`; the callers that reuse it by
`test_bounded_audio_reverse_callers`.

Run just this file locally, from the repo root:
    python tests/run_suite.py -k bounded_audio_reverse_run
"""
...
from ffmwiz.support import ext04b, ext04c, ext04c_run


from audio_reverse_fixtures import (FFMPEG, FFPROBE, RATE, TONES,
                                    goertzel, probe_seconds,
                                    requires_ffmpeg, run_tool)
...
class BoundedAudioReverse(NoLeakedArtifacts, unittest.TestCase):
    ...
    def _tone_source(self, name, codec, *, channels=1, rate=RATE, tones=TONES,
        ...
        if cover:
            ...
            if not art.exists():
                run_tool([FFMPEG, "-v", "error", "-y", "-f", "lavfi", "-i",
    ...
    def _tone_order(self, values, windows=4, rate=RATE):
        ...
        for index in range(windows):
            ...
            energies = {tone: goertzel(window, rate, tone) for tone in TONES}
    ...
    def test_a_short_join_refuses_instead_of_reporting_success(self):
        ...
        code, output, noise = self._execute(
            ...
            + [mock.patch.object(ext04c_run, "build_audio_concat_filter_command",
                                  only_the_first)])
    ...
    def test_a_failing_stage_leaves_no_scratch_behind(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            stack.enter_context(mock.patch.object(
                ext04c_run, "run_ffmpeg_with_progress", failing))
            stack.enter_context(contextlib.redirect_stdout(noise))
            code, _elapsed = FFmWiz.run_bounded_audio_reverse(
                answers, FFmWiz.build_audio_speed_reverse_command, label="check")
        self.assertEqual(1, code)
    ...
    def test_a_cancelled_run_leaves_no_scratch_behind(self):
        ...
        with contextlib.ExitStack() as stack:
            for patch in patches:
                stack.enter_context(patch)
            stack.enter_context(mock.patch.object(
                ext04c_run, "run_ffmpeg_with_progress", cancelled))
