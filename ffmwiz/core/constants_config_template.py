"""FFmWiz Mode-2 config.env template.

Holds only the CONFIG_TEMPLATE string (the annotated config.env sample written
on first run). Split out of ffmwiz/core/constants.py to keep that constants
module scannable. Leaf module: no imports. Re-exported by constants.py, so the
public API (FFmWiz.CONFIG_TEMPLATE) is unchanged.
"""
...
#   Applied in one filter chain together with crop, resize, speed and reverse.
#     rotate     90cw, 90ccw, 180
#     mirror     hflip, vflip
#     colour     gray, bright=N, contrast=N, sat=N
#     detail     denoise, sharpen, blur  (each also =light|medium|heavy)
#     fades      fadein=SECONDS, fadeout=SECONDS  (picture and sound together)
#   Example:  video_look=90cw,gray,denoise,fadein=1.5
#   Seconds are measured on the OUTPUT, so a fade lasts what you asked for even
#   with a speed change. sharpen and blur cancel each other; pick one. Leave
#   this key out entirely and Mode 2 never asks about it.
video_look=n

# video_quick: a one-shot output shape, or  n  for a normal encode. One of:
#     gif[=fps[:width]]         animated GIF (two-pass palette); default 15 fps, 480px wide
#     boomerang                  forward then the same clip reversed; keeps its sound
#     thumb[=SECONDS|HH:MM:SS]   a single frame (default format png)
#   loop=N may be added to any of them (repeats the input N extra times via
#   -stream_loop). Only one of gif/boomerang/thumb may be given at a time.
#   Example:  video_quick=gif=12:320
video_quick=n

# audio_volume: audio gain as a factor (1.5), a percentage (150%) or decibels
#   (+6dB), or  n  for no change. Accepted range 0.01-10.0 (about -40 dB to
#   +20 dB); anything outside it is refused as more likely a typo than an
#   intention. Applied AFTER loudnorm, which normalises to a target that an
#   earlier gain would undo.
audio_volume=n

# raw_ffmpeg_args: your own extra ffmpeg options, or  n  for none. Split the
#   way a shell would quote them (e.g.  -metadata title="My film"  keeps its
#   spaces), and placed LAST, immediately before the output path, so they can
#   override what the wizard chose. Options the wizard owns (-i, -vf, -c:v,
#   -map, -ss, -y, -filter_complex, and the rest) are refused, so the printed
#   command never disagrees with the summary beside it.
raw_ffmpeg_args=n

# raw_ffmpeg_valued_args: extra option NAMES that take one value, for an expert
#   option this FFmpeg build supports and FFmWiz has not listed. Comma or space
#   separated, with or without the leading dash (e.g.  brand, strict ), or  n
#   for none. FFmWiz refuses an option whose arity it does not know rather than
#   guess -- a guess turns a bare filename into a second output nobody asked
#   for -- so declaring it here is how you widen the grammar safely.
raw_ffmpeg_valued_args=n
...
# log_level: debug (default) | info | warning | error | critical. Controls what
# reaches the log FILE; the console is unaffected. Over half a typical log is
# DEBUG detail (per-line FFmpeg stderr, progress parser events, exact argv), so
# set info for a much smaller file. Keep debug when collecting evidence for a
# bug report. The FFMWIZ_LOG_LEVEL environment variable overrides this.
log_level=debug
