"""FFmWiz wizard primitives shared by the whole wizard tier.

Split out of wizard.py, which is now a pure facade over this module and
wizard_build / wizard_steps / wizard_flow / wizard_b. Those siblings need
Step, the NVENC multipass helpers, the encode-option builder and the
pixel-format analysis; while they lived in the facade every one of them had
to import it, and the facade merges their __all__ -- so importing any leaf
first re-entered the facade before that list existed (D09). Nothing here
imports wizard, so the tier is acyclic.
"""
