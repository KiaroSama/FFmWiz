...
# `_ffmpeg_float` / `_gui_atempo_chain` are re-exported, not used here:
# gui_editor_unified reaches them through the assembled namespace, and that
# namespace only carries names bound on THIS module.
from ffmwiz.gui.classic.gui_editor_speed_reverse import (  # noqa: F401
    _ffmpeg_float, _gui_atempo_chain, build_speed_reverse_mixin)
from ffmwiz.gui.classic.gui_editor_speed_transport import (
    build_speed_transport_mixin)
...
def build_speed_editor(request: dict[str, Any], media_kind: str):
    ...
    SpeedTransportMixin = build_speed_transport_mixin()
    SpeedReverseMixin = build_speed_reverse_mixin()

    class SpeedEditorWindow(SpeedTransportMixin, SpeedReverseMixin, QMainWindow):
