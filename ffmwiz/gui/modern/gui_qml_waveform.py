...
def compute_wave_key(req: dict) -> str:
    ...
    def identity(item):
        path = str(item.get("path") or item.get("input_path") or "")
        try:
            info = Path(path).stat()
            stamp = [info.st_dev, info.st_ino, info.st_size, info.st_mtime_ns]
        except OSError:
            stamp = None
        return {"path": path, "file": stamp, "duration": item.get("duration"),
                "origin": item.get("picture_clock_offset", 0.0),
                "stream": segment_audio_stream_spec(req, item),
                "has_audio": bool(item.get("has_audio", True))}

    # Separate structured fields avoid delimiter collisions in unusual paths.
    # Origin and per-segment track choice alter PCM even when duration does not.
    payload = {"version": 2, "rate": WAVE_RATE, "duration": req.get("duration"),
               "segments": [identity(item) for item in (req.get("join_segments") or [req])]}
    text = json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    return hashlib.sha256(text.encode("utf-8", "surrogatepass")).hexdigest()[:16]


# Public aliases stay available at the old import paths.
from ffmwiz.gui.gui_waveform_decode import (
