"""FFmWiz runner cluster (extracted from FFmWiz.py, method الف)."""
...
def confirm_and_run_ffmpeg(answers: dict[str, Any], cmd: list[str], label: str,
    ...
    if not appio.ask_yes_no(metadata_prompt(answers, "Start FFmpeg now?", "y/n", "y"), True):
        appio.note("FFmpeg was not started. Returning to the Metadata Editor menu.")
        return False
    rc, _elapsed = run_ffmpeg_with_progress(cmd, total_duration=None, label=label)
    if rc == 0:
        if output_path is not None:
            appio.note(f"Metadata operation finished: {output_path}")
        return True
    appio.error(f"Metadata operation failed. Full output is in: {_log_file_text()}")
    return False


__all__ = [
    'confirm_and_run_ffmpeg',
]
