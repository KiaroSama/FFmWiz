"""Shared Tk widgets/theme helpers for the classic FFmWiz editors.

These used to live in guibridge.py, which made the two Tk editors reach back
up into their own facade (`guibridge._UIPalette`, ...) -- a two-way import
that left the editors unimportable on their own. They live one tier below
guibridge now, so guibridge, guibridge_crop_tk and guibridge_cut_tk all
import DOWN into this module. guibridge re-exports every name so
`from ffmwiz.guibridge import *` is unchanged.
"""
...
from ffmwiz.core.constants import FFMWIZ_GUI_DIR_NAME
