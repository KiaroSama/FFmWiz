...
def build_unified_video_editor(request: dict[str, Any]):
    ...
    class UnifiedTimelineWidget(QWidget):
        ...
        def _snap_time_to_cti(self, seconds: float) -> float:
            ...
            tolerance = max(0.03, seconds_per_px * 12.0, 2.0 / max(1.0, self.fps))
            if abs(seconds - self.playhead) <= tolerance:
        ...
        def paintEvent(self, _event):
            ...
            for label, value, color_name in (
                ...
                line_top = wave.top() + (15 if selected else 12)
                p.setPen(QtGui.QPen(stroke, 8 if selected else 6))
                p.drawLine(QPointF(x, line_top), QPointF(x, wave.bottom() + 4))
                p.setPen(QtGui.QPen(marker_color, 5 if selected else 3))
                p.drawLine(QPointF(x, line_top), QPointF(x, wave.bottom() + 4))
                p.setBrush(QtGui.QBrush(marker_color))
                p.setPen(QtGui.QPen(stroke, 2))
                half = 12 if selected else 9
                tip = wave.top() + (15 if selected else 12)
                p.drawPolygon(QtGui.QPolygonF([
                    QPointF(x - half, wave.top() - 1),
                    QPointF(x + half, wave.top() - 1),
                    QPointF(x, tip),
                ]))
                p.setPen(QtGui.QPen(marker_color, 1))
                p.setFont(QtGui.QFont("Segoe UI Semibold", 9 if selected else 8))
                p.drawText(QRectF(x + 5, wave.top() + 4, 42, 16), Qt.AlignLeft | Qt.AlignVCenter, label)
    ...
    class UnifiedVideoEditorWindow(QMainWindow):
        def __init__(self, req):
            ...
            QtCore.QTimer.singleShot(700, self._start_waveform)
        ...
        def _build_ui(self):
            ...
            playback_outer.setContentsMargins(10, 8, 10, 8)
            playback_outer.setSpacing(6)
            playback_header = QLabel("Playback")
            playback_header.setObjectName("sectionLabel")
            playback_outer.addWidget(playback_header)
            playback = QHBoxLayout()
            playback.setSpacing(7)
            playback_outer.addLayout(playback)
            self.btn_play = QPushButton(self._icon("play", QStyle.SP_MediaPlay), " Play (Space)")
            self.btn_play.setObjectName("primary")
            self.btn_play.clicked.connect(self.toggle_playback)
            playback.addWidget(self.btn_play)
            playback.addWidget(QLabel("CTI"))
            self.cti_time_label = QLabel("00:00:00.000")
            self.cti_time_label.setObjectName("status")
            self.cti_time_label.setMinimumWidth(112)
            self.cti_time_label.setAlignment(Qt.AlignCenter)
            self.cti_time_label.setToolTip("Current timeline position.")
            playback.addWidget(self.cti_time_label)
            playback.addWidget(self._btn(" Home (Home)", lambda: self.seek(0.0), "skip_backward", QStyle.SP_MediaSkipBackward))
            for btn in (
                self._btn(" -5s (Shift+Left)", lambda: self.seek(self.current_time() - 5.0), "seek_backward", QStyle.SP_MediaSeekBackward),
                self._btn("-1s (Left)", lambda: self.seek(self.current_time() - 1.0), "seek_backward", QStyle.SP_MediaSeekBackward),
                self._btn("+1s (Right)", lambda: self.seek(self.current_time() + 1.0), "seek_forward", QStyle.SP_MediaSeekForward),
                self._btn(" +5s (Shift+Right)", lambda: self.seek(self.current_time() + 5.0), "seek_forward", QStyle.SP_MediaSeekForward),
            ):
                btn.setAutoRepeat(True)
                btn.setAutoRepeatDelay(260)
                btn.setAutoRepeatInterval(80)
                playback.addWidget(btn)
            playback.addWidget(self._btn(" End (End)", lambda: self.seek(self.duration), "skip_forward", QStyle.SP_MediaSkipForward))
            playback.addStretch(1)
            playback_more = QHBoxLayout()
            playback_more.setSpacing(7)
            playback_outer.addLayout(playback_more)
            self.btn_prev_cut_edge = self._btn(" Prev Edge (Ctrl+Alt+←)", lambda: self.seek_nearest_cut_edge(-1), "seek_backward", QStyle.SP_MediaSeekBackward)
            self.btn_prev_cut_edge.setToolTip("Go to the nearest previous cut edge (Ctrl+Alt+Left). Hold the button to repeat.")
            ...
            playback_more.addWidget(self.btn_prev_cut_edge)
            self.btn_next_cut_edge = self._btn(" Next Edge (Ctrl+Alt+→)", lambda: self.seek_nearest_cut_edge(1), "seek_forward", QStyle.SP_MediaSeekForward)
            self.btn_next_cut_edge.setToolTip("Go to the nearest next cut edge (Ctrl+Alt+Right). Hold the button to repeat.")
            ...
            playback_more.addWidget(self.btn_next_cut_edge)
            self.btn_mute = QPushButton(self._icon("volume_meter_3", QStyle.SP_MediaVolume), " Mute (M)")
            self.btn_mute.setMinimumWidth(104)
            self.btn_mute.clicked.connect(self.toggle_mute)
            playback_more.addWidget(self.btn_mute)
            volume_frame = QFrame()
            volume_frame.setObjectName("inlineControlFrame")
            volume_frame.setFixedSize(190, 38)
            volume_frame.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
            ...
            self.volume_slider.setRange(0, 100)
            self.volume_slider.setValue(60)
            self.volume_slider.setFixedWidth(128)
            ...
            self.volume_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
            volume_layout.addWidget(self.volume_label)
            playback_more.addWidget(volume_frame)
            playback_more.addStretch(1)

            cut_panel = QFrame()
            cut_panel.setObjectName("markerPanel")
            cut_box = QVBoxLayout(cut_panel)
            cut_box.setContentsMargins(10, 8, 10, 8)
            cut_box.setSpacing(6)
            cut_header = QLabel("Markers / Cuts")
            cut_header.setObjectName("sectionLabel")
            cut_box.addWidget(cut_header)
            cut_row_top = QHBoxLayout()
            cut_row_top.setSpacing(6)
            cut_row_bottom = QHBoxLayout()
            cut_row_bottom.setSpacing(6)
            cut_panel.setObjectName("markerPanel")
            self.btn_mark_in = self._btn(" Mark In (I)", self.mark_in, "mark_in", None)
            self.btn_mark_in.setObjectName("markIn")
            cut_row_top.addWidget(self.btn_mark_in)
            self.btn_mark_out = self._btn(" Mark Out (O)", self.mark_out, "mark_out", None)
            self.btn_mark_out.setObjectName("markOut")
            cut_row_top.addWidget(self.btn_mark_out)
            add_cut = self._btn("Add Cut(s) (A)", self.add_cut)
            add_cut.setObjectName("green")
            cut_row_top.addWidget(add_cut)
            ...
            cut_row_top.addWidget(self.btn_add_separator)
            self.btn_invert = self._btn("Invert Cuts (Ctrl+Shift+I)", self.invert_cuts)
            self.btn_invert.setObjectName("purple")
            cut_row_bottom.addWidget(self.btn_invert)
            self.btn_convert_marker = self._btn("Convert Mark In/Out", self.convert_selected_marker)
            self.btn_convert_marker.setObjectName("convertMarker")
            self.btn_convert_marker.setFixedWidth(116)
            self.btn_convert_marker.setToolTip("Select Mark In or Mark Out, then convert it to the opposite marker type.")
            cut_row_bottom.addWidget(self.btn_convert_marker)
            self.btn_delete_markers = self._btn("Delete Selected Marker(s) (Del)", self.delete_selected_markers)
            self.btn_delete_markers.setObjectName("danger")
            self.btn_delete_markers.setToolTip("Delete the selected Mark In or Mark Out marker.")
            cut_row_bottom.addWidget(self.btn_delete_markers)
            self.btn_delete_separator = self._btn("Delete Split (Del)", self.delete_selected_separator)
            ...
            cut_row_bottom.addWidget(self.btn_delete_separator)
            ...
            self.preview_zoom_combo.setFixedWidth(112)
            ...
            view.addWidget(self.zoom_percent_box, 1, 2)
            ...
            self.btn_crop_overlay.setToolTip("Show or hide crop handles and guide lines for normal video viewing.")
            view.addWidget(self.btn_crop_overlay, 2, 1)
            view.addWidget(self._btn("Reset View (Ctrl+0)", self.reset_view), 3, 0)
            view.addWidget(self._btn("Reset Crop (Ctrl+R)", self.reset_crop), 3, 2)

            crop_number_frame = QFrame()
            crop_number_frame.setObjectName("panel")
            crop_number_row = QGridLayout(crop_number_frame)
            crop_number_row.setContentsMargins(10, 6, 10, 6)
            crop_number_row.setHorizontalSpacing(8)
            crop_number_row.setVerticalSpacing(4)
            crop_title = QLabel("Crop margins")
            crop_title.setObjectName("sectionLabel")
            crop_number_row.addWidget(crop_title, 0, 0, 1, 5)
            self.crop_spinboxes: dict[str, QSpinBox] = {}
            for key, label, maximum in (
                ("top", "Top", self.source_h - 16),
                ("left", "Left", self.source_w - 16),
                ("right", "Right", self.source_w - 16),
                ("bottom", "Bottom", self.source_h - 16),
            ):
                ...
                spin.setToolTip(f"Exact crop {label.lower()} margin in pixels.")
                spin.valueChanged.connect(lambda _value, _key=key: self._on_crop_field_changed(_key))
                ...
                self.crop_spinboxes[key] = spin
                if key == "top":
                    crop_number_row.addWidget(label_widget, 1, 2, alignment=Qt.AlignCenter)
                    crop_number_row.addWidget(spin, 2, 2, alignment=Qt.AlignCenter)
                elif key == "left":
                    crop_number_row.addWidget(label_widget, 3, 0, alignment=Qt.AlignRight | Qt.AlignVCenter)
                    crop_number_row.addWidget(spin, 3, 1)
                elif key == "right":
                    crop_number_row.addWidget(label_widget, 3, 3, alignment=Qt.AlignRight | Qt.AlignVCenter)
                    crop_number_row.addWidget(spin, 3, 4)
                elif key == "bottom":
                    crop_number_row.addWidget(label_widget, 4, 2, alignment=Qt.AlignCenter)
                    crop_number_row.addWidget(spin, 5, 2, alignment=Qt.AlignCenter)
            crop_number_row.setColumnStretch(2, 1)
            root.addWidget(crop_number_frame)
            ...
            speed_slider_frame_layout.setSpacing(6)
            self.speed_slider = QSlider(Qt.Horizontal)
            ...
            speed_row.addWidget(speed_slider_frame, 1)
            self.reverse_box = QCheckBox("Reverse")
            ...
            self.editor_splitter.setSizes([520, 260])

            compact_toolbar = QHBoxLayout()
            compact_toolbar.setSpacing(8)
            compact_toolbar.addWidget(playback_panel, 2)
            compact_toolbar.addWidget(cut_panel, 4)
            compact_toolbar.addWidget(view_panel, 2)
            root.addLayout(compact_toolbar)
            ...
            root.addWidget(self.timeline_resize_grip)
            ...
            self.tip_label = QLabel(
                "Tip: Space plays/pauses; right-click the preview to play/pause. Hand Tool pans the preview and double-click resets the view. "
                "Zoom Tool click/drag zooms the preview; Ctrl+(+)/Ctrl+(-) zooms the preview; +/- zooms the timeline. Drag the CTI horizontally to seek and upward/downward to zoom the timeline. "
                "I/O set Mark In/Out, A adds a cut, S adds a Split point, Delete removes the selected cut or Split, Ctrl+Shift+I inverts cuts, Ctrl+U toggles the crop overlay, Ctrl+Z/Ctrl+Y undo/redo."
            )
        ...
        def _on_zoom_slider(self, value):
            ...
            self.timeline.set_zoom_ratio(ratio, focus_time=self.timeline.playhead)
            self._sync_timeline_controls()
        ...
        def _start_waveform(self):
            ...
            if self.join_segments:
                ...
                for idx, _segment in enumerate(self.join_segments):
                    filters.append(f"[{idx}:a:0]aformat=channel_layouts=mono,aresample=12000,asetpts=PTS-STARTPTS[a{idx}]")
                filters.append(
                    f"{''.join(labels)}concat=n={len(self.join_segments)}:v=0:a=1,"
                    "showwavespic=s=5200x720:split_channels=0:colors=0a74ff:scale=sqrt:draw=scale[wave]"
                )
                args.extend(["-filter_complex", ";".join(filters), "-map", "[wave]"])
            else:
                args.extend([
                    "-i", str(self.request.get("input_path") or ""),
                    "-filter_complex", "[0:a:0]aformat=channel_layouts=mono,aresample=12000,showwavespic=s=5200x720:split_channels=0:colors=0a74ff:scale=sqrt:draw=scale[wave]",
                    "-map", "[wave]",
                ])
            args.extend([
                "-frames:v", "1",
